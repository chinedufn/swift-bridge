...
const NOTICE: &str = "// File automatically generated by swift-bridge.";
            ...
            match ty {
                TypeDeclaration::Shared(ty) => match ty {
                    SharedTypeDeclaration::Struct(ty_struct) => {
                        ...
                        let maybe_fields = if !fields.is_empty() {
                        ...
                        let ty_decl = format!(
                            r#"typedef struct {SWIFT_BRIDGE_PREFIX}${name} {{{maybe_fields}}} {SWIFT_BRIDGE_PREFIX}${name};
...
{derive_debug_impl}{maybe_vec_support}"#
                            ...
                            let enum_decl = format!(
                                r#"{variant_fields}union {ffi_union_name} {ffi_union_field_names};
...
{derive_debug_impl}{maybe_vec_support}"#,
                ...
                TypeDeclaration::Opaque(ty) => {
                    ...
                    if ty.attributes.hashable {
                        ...
                        let hash_ty =
                            format!("uint64_t __swift_bridge__${ty_name}$_hash(void* self);");
                        header += &hash_ty;
                    }
                    ...
                    if let Some(copy) = ty.attributes.copy {
                        ...
                        let option_ty_decl = format!(
                            "typedef struct {c_option_ty_name} {{ bool is_some; {c_ty_name} val; }} {c_option_ty_name};",
                        );
                        ...
                        let ty_decl =
                            format!("typedef struct {ty_name} {ty_name};");
                        header += &ty_decl;
                        header += "\n";
                    ...
                    if ty.attributes.equatable {
                        ...
                        let equal_ty = if ty.attributes.copy.is_some() {
                            ...
                            format!(
                                "bool __swift_bridge__${ty_name}$_partial_eq(void* lhs, void* rhs);"
                            )
                        };
                        bookkeeping.includes.insert("stdint.h");
                        bookkeeping.includes.insert("stdbool.h");
